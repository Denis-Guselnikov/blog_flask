from flask import Blueprint, request, url_for, redirect, flash, render_template, session, g, request
import sqlite3

admin = Blueprint('admin', __name__, template_folder='templates', static_folder='static')  # реализует эскиз приложения


def login_admin():
    session['admin_logged'] = 1  # Авторизация через сессию что-бы меньше было привязки к flsite
...
def logout_admin():
    session.pop('admin_logged', None)   # .pop удаляет admin_logged
